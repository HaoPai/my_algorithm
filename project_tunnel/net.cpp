...
void CommuSocket::get_local_info()
{
    char local_ip[20];
    sockaddr_in address;
    socklen_t addr_len = sizeof(address);
    getsockname(sock_fd,(sockaddr *)&address,&addr_len);
    inet_ntop(AF_INET, &address.sin_addr, local_ip, sizeof(local_ip));
    cout << local_ip << ":" << ntohs(address.sin_port) << endl;
...
void ServerSocket::accept_client(CommuSocket* &commu)
{
    ...
    cout << endl << "Server: acepted from "<< inet_ntoa(client_addr.sin_addr)<<"port" << client_addr.sin_port << endl;
...
NetPeer ServerSocket :: get_client()
{
    return client;
}
